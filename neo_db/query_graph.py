...
def query(name):
    data = graph.run(
        "match (p)-[r]->(n:Concept) where n.name =~ '(?i)%s' return  p.name,r.relation,n.name,p.cate,n.cate\
            Union all\
        match (p:Concept)-[r]->(n) where p.name =~ '(?i)%s' return p.name, r.relation, n.name, p.cate, n.cate" % (name, name)
    )
    data = list(data)
...
def get_KGQA_answer(array):
    ...
    for i in range(len(array) - 1):
        ...
        data = graph.run(
            "match(p:Concept)-[r:%s{relation: '%s'}]->(n) where p.name=~'(?i)%s'  return  p.name,n.name,r.relation,p.cate,n.cate" % (
